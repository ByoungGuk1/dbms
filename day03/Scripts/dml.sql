-- PARENT
-- ID(PK), 이름, 나이, 주소, 핸드폰, 성별

CREATE SEQUENCE SEQ_PARENT;
CREATE TABLE TBL_PARENT(
	ID NUMBER CONSTRAINT PK_PARENT PRIMARY KEY,
	PARENT_NAME VARCHAR2(255) NOT NULL,
	PARENT_AGE NUMBER,
	PARENT_ADDRESS VARCHAR2(255) NOT NULL,
	PARENT_PHONE VARCHAR2(255) NOT NULL,
	PARENT_GENDER CHAR(10),
	CONSTRAINT GENDER_TYPE CHECK(PARENT_GENDER IN ('M','W'))
);

-- 에스터리스크, 와일드카드 (*)
-- SELECT 와 FROM 은 세트
-- SELECT 컬럼명 1, 컬럼명2...
-- FROM 테이블1, 테이블2 ... //-> 가능은 하지만 테이블 하나씩 가져오기

SELECT *
FROM TBL_PARENT;

--DROP TABLE TBL_PARENT;
--DROP SEQUENCE SEQ_PARENT;

-- INSERT 와 INTO 는 세트
-- INSERT INTO <TABLE>(컬럼명1, 컬럼명2, ...)
-- VALUES (값1, 값2, ...)
-- 값과 컬럼의 위치 잘 확인하기

INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER )
VALUES(SEQ_PRODUCT.NEXTVAL, '홍길동', 30, '경기도 과천시','010-1234-5678','M');

INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER )
VALUES(SEQ_PRODUCT.NEXTVAL, '장보고', 37, '경기도 군포시','010-4456-1123','M');

INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER )
VALUES(SEQ_PRODUCT.NEXTVAL, '김영희', 45, '경기도 안산시','010-8813-6644','W');

INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER )
VALUES(SEQ_PRODUCT.NEXTVAL, '김철수', 50, '서울시 관악구','010-8813-6644','M');

INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER )
VALUES(SEQ_PRODUCT.NEXTVAL, '이태희', 42, '경기도 고양시','010-7711-2453','W');

-- WHERE 조건절
-- 성별이 남성인 부모 조회
SELECT ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER
FROM TBL_PARENT
WHERE PARENT_GENDER = 'M';

-- 40대 이상인 부모 중 여성만 가져오기
SELECT ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER
FROM TBL_PARENT
WHERE PARENT_AGE >= 40 AND PARENT_GENDER = 'W';

-- LIKE (=)
  /*
  * LIKE: 포함된 문자열 값을 찾고, 문자의 개수도 제한을 줄 수 있다.
  * 
  * %: 모든 것
  * _: 글자 수
  * 
  * 예시
    '%A' : A로 끝나는 모든 값(NDFLSNJFA, 123NFJA, ...)
    'A%' : A로 시작하는 모든 값(AA, ANDKFS, AJD, ...)
    'A__' : A로 시작하며 3글자인 값(ABC, AAA, ADD, ...)
    '_A' : A로 끝나며 2글자인 값(FA, CA, DA, ...)
    '%A%' : A가 포함된 값(APPLE, BANANA, TASK, ...)
  * 
  * */

-- 경기도에 거주하는 부모의 데이터 조회
SELECT *
FROM TBL_PARENT
WHERE PARENT_ADDRESS LIKE '%경기%';

-- 이름이 --희로 끝나는 부모의 데이터를 조회
SELECT *
FROM TBL_PARENT
WHERE PARENT_NAME LIKE '%희';

-- 이름이 거주지가 과천이 아닌 부모의 데이터를 조회
SELECT *
FROM TBL_PARENT
WHERE PARENT_ADDRESS NOT LIKE '%과천%';

----------------------------------------------------
-- 집계함수
  /*
  * 집계 함수
  * 
  * 평균 AVG()
  * 최대값 MAX()
  * 최소값 MIN()
  * 총 합 SUM()
  * 개수 COUNT()
  * 
  * */

-- 집계함수는 쓸 수 있는 위치가 정해져 있다.
-- 1. SELECT절
-- 2. HAVING절

-- SELECT 순서가 변경되면 안됨.(생략은 가능)
-- SELECT
-- FROM
-- WHERE
-- GROUP BY
-- HAVING
-- ORDER BY


CREATE SEQUENCE SEQ_FIELD_TRIP;
CREATE TABLE TBL_FIELD_TRIP(
	ID NUMBER CONSTRAINT PK_FIELD_TRIP PRIMARY KEY,
	FIELD_TRIP_TITLE VARCHAR2(255),
	FIELD_TRIP_CONTENT VARCHAR(255),
	FIELD_TRIP_NUMBER NUMBER
);

INSERT INTO TBL_FIELD_TRIP
VALUES (SEQ_FIELD_TRIP.NEXTVAL, '어서와 매미농장', '매미 잡자 매미 잡자', 20);
INSERT INTO TBL_FIELD_TRIP
VALUES (SEQ_FIELD_TRIP.NEXTVAL, '아이스크림 빨리 먹기 대회', '아이스크림 누가 잘 먹나', 100);
INSERT INTO TBL_FIELD_TRIP
VALUES (SEQ_FIELD_TRIP.NEXTVAL, '고구마 심기', '고구마가 왕 커요', 10);
INSERT INTO TBL_FIELD_TRIP
VALUES (SEQ_FIELD_TRIP.NEXTVAL, '숭어 얼음 낚시', '숭어 잡자 추워도 참아!', 80);
INSERT INTO TBL_FIELD_TRIP
VALUES (SEQ_FIELD_TRIP.NEXTVAL, '커피 체험 공장', '커피 빈 객체화', 60);
INSERT INTO TBL_FIELD_TRIP
VALUES (SEQ_FIELD_TRIP.NEXTVAL, '치즈 제작하기', '여기 치즈 저기 치즈 이쪽 치즈 저쪽 치즈', 5);
INSERT INTO TBL_FIELD_TRIP
VALUES (SEQ_FIELD_TRIP.NEXTVAL, '승마 체험', '이리야!', 9);

SELECT *
FROM TBL_FIELD_TRIP;

-- 1. SELECT절
-- 해당 필드의 ID 갯수 확인하기
SELECT COUNT(ID)
FROM TBL_FIELD_TRIP;

-- 해당 필드의 별명 붙이기 AS (ALIAS)
-- 컬럼 명은 "" (더블 쿼터)로
SELECT SUM(FIELD_TRIP_NUMBER) AS "총 수용인원"
FROM TBL_FIELD_TRIP;

-- 체험학습 평균 인원수
-- 반올림 => ROUND(값, 자릿수)
SELECT ROUND(AVG(FIELD_TRIP_NUMBER), 1) AS "평균 인원"
FROM TBL_FIELD_TRIP;

-- 반올림
SELECT ROUND(AVG(FIELD_TRIP_NUMBER)) AS "평균 인원"
FROM TBL_FIELD_TRIP;

-- 내림 -> FLOOR
SELECT FLOOR(AVG(FIELD_TRIP_NUMBER)) AS "평균 인원"
FROM TBL_FIELD_TRIP;

-- 올림 -> CEIL
SELECT CEIL(AVG(FIELD_TRIP_NUMBER)) AS "평균 인원"
FROM TBL_FIELD_TRIP;

-- '체험' 글자를 포함하는 체험학습의 총 인원수를 조회
SELECT SUM(FIELD_TRIP_NUMBER) AS "'체험' 글자를 포함한 체험의 총 인원수"
FROM TBL_FIELD_TRIP
WHERE FIELD_TRIP_TITLE LIKE '%체험%';

-- 제목에 고구마 또는 아이스크림이 들어간 체험 학습의 평균 인원수
SELECT AVG(FIELD_TRIP_NUMBER) AS "고구마 또는 아이스크림이 들어간 체험 학습의 평균 인원수"
FROM TBL_FIELD_TRIP
WHERE FIELD_TRIP_TITLE LIKE '%고구마%' OR FIELD_TRIP_TITLE LIKE '%아이스크림%';

-- 체험학습 제목이 '기'로 끝나는 체험 학습의 최소 인원수
SELECT MIN(FIELD_TRIP_NUMBER) AS "제목이 '기'로 끝나는 체험 학습의 최소 인원수"
FROM TBL_FIELD_TRIP
WHERE FIELD_TRIP_TITLE LIKE '%기';

-----

CREATE SEQUENCE SEQ_PRODUCT;
CREATE TABLE TBL_PRODUCT(
	ID NUMBER CONSTRAINT PK_PRODUCT PRIMARY KEY,
	PRODUCT_NAME VARCHAR2(255),
	PRODUCT_PRICE NUMBER,
	PRODUCT_STOCK NUMBER
);

SELECT * FROM TBL_PRODUCT;

INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 1000, 20);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 2000, 40);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 4000, 30);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 1000, 60);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 3000, 80);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 2000, 10);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 5000, 20);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 2000, 15);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 2000, 65);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 5000, 40);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 3000, 17);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 1000, 25);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 3000, 25);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 4000, 20);
INSERT INTO TBL_PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '배승원의 그램', 2000, 40);

-----

-- 2. HAVING절
-- GROUP BY

-- 평균 인원보다 인원수가 많은 체험학습을 조회 <아래의 코드는 에러코드>
--SELECT *
--FROM TBL_FIELD_TRIP
--WHERE AVG(FIELD_TRIP_NUMBER) <= FIELD_TRIP_NUMBER;

-- GROUP BY 로 지정한 컬럼만 조회할 수 있다
-- 그룹 함수를 조회할 수 있다.
SELECT PRODUCT_NAME, COUNT(PRODUCT_NAME)
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME;

-- 가격대별 개수 구하기
SELECT PRODUCT_NAME, PRODUCT_PRICE
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_PRICE;

SELECT
	PRODUCT_NAME AS "상품 명",
	PRODUCT_PRICE AS "상품 가격",
	COUNT(ID) AS "상품 개수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_PRICE;

-- HAVING <GROUP BY 와 한 세트로 외우기>
-- WHERE 절과 똑같은 GROUP BY 의 조건절

SELECT
	PRODUCT_NAME AS "상품 명",
	PRODUCT_PRICE AS "상품 가격",
	COUNT(ID) AS "상품 개수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_PRICE
HAVING COUNT(ID) >= 3;

SELECT
	PRODUCT_NAME AS "상품 명",
	PRODUCT_PRICE AS "상품 가격",
	COUNT(ID) AS "상품 개수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_PRICE
HAVING AVG(PRODUCT_PRICE) > 2000;


SELECT * FROM TBL_PRODUCT;
-- 1>  가격대별 재고 수
SELECT 
	PRODUCT_NAME AS "상품 명", 
	PRODUCT_PRICE || '개' AS "상품 가격", 
--	문자열을 연결 시킬 때 || 'STR'
	COUNT(ID) AS "상품 수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_PRICE;
-- 2> 재고가 50개 이상인 개수
SELECT COUNT(ID)
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK >= 50;
-- 3> 재고가 30개 미만인 상품의 가격 평균 가격
SELECT 
	PRODUCT_NAME AS "상품 명", 
	PRODUCT_STOCK AS "상품 재고", 
	ROUND(AVG(PRODUCT_PRICE)) AS "상품의 평균 가격"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_STOCK
HAVING PRODUCT_STOCK < 30;

-- 4> 재고가 30개 미만 상품의 가격대별 개수가 2개 미만인 총 가격 합계
SELECT
	PRODUCT_NAME AS "상품 명",
	PRODUCT_PRICE || '원' AS "가격대",
	COUNT(ID) || '개'  AS "개수",
	SUM(PRODUCT_PRICE) || '원' AS "가격의 합"
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK < 30
GROUP BY PRODUCT_PRICE, PRODUCT_NAME
HAVING COUNT(ID) <= 2;

-- WHERE 컬럼명 IN (값1, 값2, 값3, ...);
SELECT *
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK IN ('20', '30', '40');

-- 가격이 1000, 3000, 5000 인 상품만 조회
SELECT
	PRODUCT_NAME AS "상품 명",
	PRODUCT_PRICE AS "상품 가격",
	PRODUCT_STOCK AS "상품 재고"
FROM TBL_PRODUCT
WHERE PRODUCT_PRICE IN ('1000', '3000', '5000');

-- 상품 재고가 15, 20, 25인 상품 중
-- 상품 재고별 개수와 평균 가격 구하기
SELECT
	PRODUCT_NAME AS "상품 명",
	SUM(PRODUCT_STOCK) AS "재고 총 합",
	CEIL(AVG(PRODUCT_PRICE)) || '원' AS "평균 가격"
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK IN ('15', '20', '25')
GROUP BY PRODUCT_NAME, PRODUCT_STOCK;

-- 모든 상품의 재고와 가격의 합
SELECT
	PRODUCT_NAME AS "상품 명",
	SUM(PRODUCT_PRICE) + SUM(PRODUCT_STOCK) AS "상품의 재고와 가격의 합"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME;

-- 상품 가격대별 기대수익
SELECT
	PRODUCT_NAME AS "상품 명",
	PRODUCT_PRICE AS "상품 가격",
	SUM(PRODUCT_STOCK) AS "상품의 재고",
	PRODUCT_PRICE * SUM(PRODUCT_STOCK) AS "상품의 기대 수익"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_PRICE;

-- 가격대별 만원으로 살 수 있는 배승원의 노트북 개수
SELECT
	PRODUCT_NAME AS "상품 명",
	PRODUCT_PRICE || '원' AS "상품 가격",
	FLOOR(10000 / PRODUCT_PRICE) || '개' AS "구매 수량"
FROM TBL_PRODUCT
GROUP BY PRODUCT_NAME, PRODUCT_PRICE
HAVING PRODUCT_NAME LIKE '배승원의 그램';
