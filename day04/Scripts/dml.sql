...
-- 쿼리문 안에 여러개의 쿼리를 값으로서 사용 할 수 있게 하는것
-- 성능적으로 좋지는 않다.
...
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK > (
	SELECT ROUND(AVG(PRODUCT_STOCK)) 
	FROM TBL_PRODUCT
)
ORDER BY ID DESC;
...
-- SELECT 
SELECT TP.PRODUCT_NAME, TP.PRODUCT_PRICE, (
	SELECT AVG(PRODUCT_PRICE) 
	FROM TBL_PRODUCT
	WHERE PRODUCT_STOCK = 20
) AS STOCK_AVG
...
INSERT INTO TBL_USER 
VALUES(SEQ_USER.NEXTVAL, 'kyh1234@naver.com', '1234', '서울시 마포구', '2002-11-11');
...
-- 5) 가장 나이가 어린 사용자가 작성한 게시글들 조회, ROWNUM 이용
SELECT *
FROM TBL_POST
WHERE USER_ID IN (
	SELECT ID
